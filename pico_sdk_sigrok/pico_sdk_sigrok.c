...
// forced_test_mode is a special mode that puts the device into an active
// sampling state out of reset.  It is used for a quick way to debug features
// without needed pulseview or sigrok-cli to initiate a transfer. The enable
// means we will start the mode once, the run indicates it is currently running.
// To enable the feature, set forced_test_mode_en to true, but leave
...
#endif // PIN_TEST_MODE
// Number of bytes stored as DMA per slice, must be 1,2 or 4 to support aligned
// access This will be be zero for 1-4 digital channels.
uint8_t d_dma_bps;
...
// at slightly different times, and based on channel configurations an adc or
// pio may not be enabled so dedicate a maintence channel for each.
...
// Keeps tracks of all interrupts we received.  Sometimes we might get an analog
// and not a digital DMA int (or vice versa) and need to exit that handler but
// need to clear out pending interrupt state so this stores what we have seen.
...
void print_DMA() {
  ...
  // of the lowest 8 DMA controllers. Note that relative to RP2040, the RP2350
  // moved the location of busy and chain_to (and maybe others)
  /*
  ...
  if (tud_ready()) {
    for (int i = 0; i < length;) {
      ...
      if (n) {
        ...
        tud_task();
        tud_cdc_write_flush();
        //                if (!tud_cdc_connected() || -replaced per pull request
        //                63
        if (!tud_ready() ||
...
void send_slice_init(sr_device_t *d, uint8_t *dbuf) {
  ...
  if ((d->cont == false) && ((d->scnt + samp_remain) > (d->num_samples))) {
    ...
    d->scnt += samp_remain;
    // Dprintf("SSIa sph %d scnt %d lval 0x%X rem %d ns
    // %d\n\r",d->samples_per_half,d->scnt,lval,samp_remain,d->num_samples);
  } else {
    d->scnt += d->samples_per_half;
    // Dprintf("SSIb sph %d scnt %d lval 0x%X rem %d ns
    // %d\n\r",d->samples_per_half,d->scnt,lval,samp_remain,d->num_samples);
  }
}

// This is an optimized transmit of trace data for configurations with 4 or
// fewer digital channels and no analog.  Run length encoding (RLE) is used to
// send counts of repeated values to effeciently utilize USB CDC link bandwidth.
// This is the only mode where a given serial byte can have both sample
// information and RLE counts. Samples from PIO are dma'd in 32 bit words, each
// containing 8 samples of 4 bits (1 nibble). RLE Encoding: Values 0x80-0xFF
// encode an rle cnt of a previous value with a new value:
...
// For longer runs, an RLE only encoding uses decimal values 48 to 127 (0x30 to
// 0x7F) as x8 run length values of 8..640. All other ascii values (except from
// the abort and the end of run byte_cnt) are reserved.
uint32_t send_slices_D4(sr_device_t *d, uint8_t *dbuf) {
  ...
  // Don't optimize the first word (eight samples) perfectly, just send them to
  // make the for loop easier, and setup the initial conditions for rle tracking
  cptr = (uint32_t *)&(dbuf[0]);
  cword = *cptr;
#ifdef D4_DBG
  Dprintf("Dbuf %p cptr %p data 0x%X\n\r", (void *)&(dbuf[0]), (void *)cptr,
          cword);
#endif
  lword = cword;
  for (int j = 0; j < 8; j++) {
  ...
  // 8 samples in it, especially if pulseview is running.  But for some useages
  // it may be only 8 so exit on the first 8. This is just mostly to prevent
  // underflow of samp_remain when we subtract 8 from it.
...
#endif
    // Send maximal RLE counts in this outer section to the txbuf, and if we
    // accumulate a few of them push to the device so that we don't accumulate
    // large numbers of unsent RLEs.  That allows the host to process them
    // gradually rather than in a flood when we get a value change.
    while (rlecnt >= 640) {
      txbuf[txbufidx++] = 127;
      rlecnt -= 640;
      if (txbufidx > 3) {
...
// parsing digital sample data.  This function is correct for all uses, but if
// included the compiled code is substantially slower to the point that digital
// only transfers can't keep up with USB rate.  Thus it is only used by the
// send_slices_analog which is already limited to 500khz, and in the starting
// send_slice_1/2/4
...
// Not used for send_analog because it doesn't use RLE, and not used for D4
// because it has a different RLE encoding
...
// substantially.  Thus each function has a 1,2, or 4B aligned read
// respectively. We can't just always read a 4B value because the core doesn't
// support non-aligned accesses. These must be marked noinline to ensure they
// remain separate functions for good performance 1B is 5-8 channels
...
// It is also used with 17-26 channels in DIG_26_MODE, and 17-32 channels in
// DIG_32_MODE
...
// All digital channels for one slice are sent first in 7 bit bytes using values
// 0x80 to 0xFF Analog channels are sent next, with each channel taking one 7
// bit byte using values 0x80 to 0xFF. This does not support run length encoding
// because it's not clear how to define RLE on analog signals This functional
// will only be called in BASE_MODE, as neither DIG_26_MODE or DIG_32 mode have
// analog support
...
// This function monitors the dma interrupt handler outputs to send the
// remainder of a full DMA buffer.
void send_half(void) {
  bool sendlower;
  ...
  if (dma_halves > num_halves) {
    ...
    // Dprintf("d buffers %d %d
    // %d\n\r",dev.dbuf0_start,dev.dbuf1_start,dbuf_start); Dprintf("a buffers
  ...
  // If we ever recieve a usb_plus, consider all samples to be sent, even if not
  // in continuous mode
  if (dev.usb_plus) {
    ...
    if ((dev.scnt >= dev.num_samples) || (dev.cont == true)) {
      ...
      // Even with dma disabled, we still might have one more half buffer to
      // send so allow this loop to be called again for the remaining half
      // buffer
      if (dma_halves - num_halves == 1) {
        // Dprintf("ONEMORE\n\r");
      } else {
        if (mask_xfer_err == false) {
          // If we have more than one extra we have some kind of
          // overflow/error/abort etc that isn't expected.
          Dprintf("Unexpected state cnt %d %d halves %d %d %d\n\r", dev.scnt,
                  dev.num_samples, exp_halves, dma_halves, num_halves);
          dev.state = ABORTED;
        }
      } // else
    } // DMA_DONE
...
// Handle interrupts generated by ADC or PIO.  If both are enabled they may come
// in either order, so wait for both if only one is seen.
void dma_int_handler() {
  int sts;
  // Have we detected any cases were dma should be turnned off and interrupts
  // disabled? this includes error/abort and non error/abort cases
  bool dma_done = false;
  bool partial = true;
  sts = dma_hw->ints0;
  currintmask |= sts;
  ...
  // The "+" from the host ends a transmit, in both continuous and
  ...
  // lower halves and an IRQ for either of the upper halves that we have
  // overflowed. This is rather exceptional as it means we managed to finish
  // DMAs for both halves and only called the interrupt handler once.
  else if ((mask_xfer_err == false) &&
           ((currintmask & h0intmask) && (currintmask & h1intmask))) {
    Dprintf("Int Overflow0 a %d b %d c %d d %d e %d halves %d %d masks %X %X "
            "%X \n\r",
            acnt, bcnt, ccnt, dcnt, ecnt, dma_halves, num_halves, currintmask,
            h0intmask, h1intmask);
    dma_done = true;
    dev.state = ABORTED;
  }
  // If we haven't detected any errors process the current interrupt mask
  // Note that in very high sample rates with low number of samples we may reach
  // this interrupt handler with both halves being valid, so check and clear
  ...
  // but not both, in that case exit the handler waiting for the other to catch
  // up
  if (partial) {
    // Dprintf("PRT %X\n",currintmask);
  }

  // This 2nd overflow check says if dma_halves is more than one ahead of
  // num_halves then we are starting to overwrite a buffer we are sending. Note
  // that it is after we increment dma_halves .
  if ((mask_xfer_err == false) && (dma_halves - num_halves > 1)) {
    Dprintf("Int Overflow1 a %d b %d c %d d %d e %d halves %d %d masks %X %X "
            "%X \n\r",
            acnt, bcnt, ccnt, dcnt, ecnt, dma_halves, num_halves, currintmask,
            h0intmask, h1intmask);
    dma_done = true;
  ...
  // and the bytecnt etc, so it's just enough to stop dma transfers and
  // interrupts
  if (dma_done) {
    // Dprintf("DMADNE %X\n\r",sts);
    currintmask = 0;
    ...
    // Aborting the maintenance channels is generally safe because they are only
    // one DMA operation and aborting them is sufficient to break the chain and
    // prevent new DMA operations from overwritting data that hasn't been sent
    // across USB. dma_channel_abort(admachan0); dma_channel_abort(admachan1);
...
// from a counter and write to the GPIOs. However, DMA isn't allowed access to
// SIO. We could also use the PIOs, but PIOs are used for the main functionaliy
// and PIO clocks are modified based on sample rate, so using the normal timer
// ensures more predictable timing. Note: This function makes no attempt to
// determine which pins are enabled. Note: During transitions between DMA
// engines the delay between timers may jump from 100us to ~300us, thus
// "stretching" the sample values of the test.  Not sure why such a large delta
// exists, but just be aware of the limitation. It may only happen with high
// rates of serial print outs...
bool pin_test_timer_callback(__unused struct repeating_timer *t) {
  ...
  // In case the user forgets to not drive the pins, only drive the test signal
  // when not idle to limit drive fights.
  if (dev.state != IDLE) {
    sio_hw->gpio_out = pin_test_cnt;
    // mask        value
    gpio_set_dir_masked(
        PIN_TEST_MASK,
        PIN_TEST_MASK); // masked set per pin.  1 is output, 0 is input
  } else {
    sio_hw->gpio_out = 0;
    // mask        value
    gpio_set_dir_masked(PIN_TEST_MASK,
                        0); // masked set per pin.  1 is output, 0 is input
  }
  return true;
}
void core1_entry() {
  ...
  // gpio_set_dir_masked(PIN_TEST_MASK,PIN_TEST_MASK); //masked set per pin.  1
  // is output, 0 is input
...
#endif
  // Early CDC IO code had lots of sleep statements, but the TUD code seems to
  // have sufficient checks that this isn't needed, but it doesn't hurt...
  sleep_us(100000);
// GPIOs 26 through 28 (the ADC ports) are on the PICO, GPIO29 is not a pin on
// the PICO Note that digital only modes don't block all configuration related
// to ADC, but does enough to ensure we can properly sample the pins digitally.
#ifdef BASE_MODE
  adc_gpio_init(26);
  adc_gpio_init(27);
  adc_gpio_init(28);
  ...
  Dprintf("DMA Channels A0 %d P0 %d A1 %d P1 %d M %d %d %d %d\n", admachan0,
          pdmachan0, admachan1, pdmachan1, amaintchan0, amaintchan1,
          pmaintchan0, pmaintchan1);
  ...
  // and ADMA0->AMAINT0->ADMA1->AMAINT1->ADMA0, except in mask_xfer_err case
  // where we know we will only have two half buffers and will likely fill them
  // way before we can process them.  In that case we end the chaining by not
  // doing a chaint_to from maintenance 1. The mask_xfer_err override is handled
  // as part of the init for each sample run
  ...
  // The DMA controller must read across the common bus to read the PIO fifo so
  // enabled both reads and write
  bus_ctrl_hw->priority =
      BUSCTRL_BUS_PRIORITY_DMA_W_BITS | BUSCTRL_BUS_PRIORITY_DMA_R_BITS;

  init(&dev);
  // Since RP2040 is 32 bit this should always be 4B aligned, and it must be
  // because the PIO does DMA on a per byte basis If either malloc fails the
  // code will just hang
...
// It also helps above Dprintf/UART conflicts between the two cores
  ...
  while (1) {
    ...
    // It must be placed immediately before the dev.sending/started check just
    // below
    ...
    if (dev.state == STARTED) {
      ...
      // because it specifies a fixed set of frequencies, but sigrok cli can
      // still odd ones.
      dev.sample_rate >>= 1;
      dev.sample_rate <<= 1;
      ...
      // thus a_chan_cnt is multiplied by 2 Nibble size storage is only allow
      // for D4 mode with no analog channels enabled For instance a D0..D5 with
      // A0 would give 1/2 the storage to digital and 1/2 to analog
      uint32_t d_nibbles, a_nibbles,
          t_nibbles;         // digital, analog and total nibbles
      ...
      // total buf size must be a multiple of a_nibbles*2, d_nibbles*8, and
      // t_nibbles so that division is always in whole samples. Also set a
      // multiple of 32  because the dma buffer is split in half, and the PIO
      // does writes on 4B boundaries, and then a 4x factor for any other
      ...
      uint32_t dig_samples_per_chunk =
          (d_nibbles) ? dig_bytes_per_chunk * 2 / d_nibbles : 0;
      uint32_t chunk_samples =
          d_nibbles ? dig_samples_per_chunk : (chunk_size * 2) / (a_nibbles);
      ...
      uint32_t chunks_needed =
          ((dev.num_samples / chunk_samples) + 2) & 0xFFFFFFFE;
      Dprintf("Initial buf calcs nibbles d %d a %d t %d \n\r", d_nibbles,
              a_nibbles, t_nibbles);
      Dprintf("chunk size %d(bytes) samples per chunk %d total chunks in both "
              "halves %d chunks needed %d\n\r",
      ...
      // If all of the samples we need fit in two half buffers or less then we
      // can mask the error logic that is looking for cases where we didn't send
      // one half buffer to the host before the 2nd buffer ended because we only
      // use each half buffer once.
      mask_xfer_err = false;
      // If requested samples are smaller than the buffer, reduce the size so
      // that the transfer completes sooner. Also, mask the sending of aborts if
      // the requested number of samples fit into RAM Don't do this in
      // continuous mode as the final size is unknown
      if (dev.cont == false) {
      ...
      // In mask_xfer_err mode we don't want the 2nd half to trigger back to the
      // 1st half and overwrite it's data, so set the maintenace config1's to
      // themselves to disable chaining
      if (mask_xfer_err) {
      ...
      if (dev.cont == false && (dev.num_samples % dev.samples_per_half))
        exp_halves++;
      Dprintf(
          "Final sizes d %d a %d mask err %d samples per half %d exp %d\n\r",
      ...
      *adcfcs |= 0xC00;
      ...
      //	  Dprintf("starting d_nps %u a_chan_cnt %u d_size %u a_size %u
      // a_mask %X\n\r"
      ...
      Dprintf("start offsets d0 0x%X d1 0x%X a0 0x%X a1 0x%X samperhalf %u\n\r",
              dev.dbuf0_start, dev.dbuf1_start, dev.abuf0_start,
              dev.abuf1_start, dev.samples_per_half);
...
//           }
      ...
      systick_idx = 0;
#endif // PIN_TEST_MODE
       // Dprintf("starting data buf values 0x%X
      ...
      if (dev.a_chan_cnt) {
        ...
        //-Values below 96 are clamped to 96 because a conversion takes a
        // minimum of 96 cycles. It is also import to subtract one from the
        // desired divisor because the period of ADC clock is 1+INT+FRAC/256 For
        // the case of a requested 500khz clock, we would normally write
        ...
        adc_frac_int = (uint8_t)(((48000000ULL % dev.sample_rate) * 256ULL) /
                                 dev.sample_rate);
        if (adcdivint <= 96) {
          ...
          *adcdiv = ((adcdivint - 1) << 8) | adc_frac_int;
          Dprintf("adcdiv %u frac %d adcdivint %d\n\r", *adcdiv, adc_frac_int,
                  adcdivint);
          // This is needed to clear the AINSEL so that when the round robin
          // arbiter starts we start sampling on channel 0
          adc_select_input(0);
          adc_set_round_robin(dev.a_mask & 0x7);
          //             en, dreq_en,dreq_thresh,err_in_fifo,byte_shift to 8 bit
          adc_fifo_setup(true, true, 1, false, true);
          // set adc0 to immediate trigger (but without adc_run it shouldn't
          // start) adc1 and the maintenance aren't triggered because they are
          // chained to each other
          //                       channel, config, write_addr,
          //                       read_addr,transfer_count,trigger)
          dma_channel_configure(admachan0, &acfg0,
                                &(capture_buf[dev.abuf0_start]), &adc_hw->fifo,
                                dev.a_size, true);
          dma_channel_configure(admachan1, &acfg1,
                                &(capture_buf[dev.abuf1_start]), &adc_hw->fifo,
                                dev.a_size, false);
          // The maintenance DMA for ADC reads the capture_buff offset value and
          // updates the ADC DMAs with it
          amaddrs[0] = (uint32_t *)&capture_buf[dev.abuf0_start];
          amaddrs[1] = (uint32_t *)&capture_buf[dev.abuf1_start];
          // This is about as close to a common/portable address offset
          // definition between devices to find the offset of the
          // write_addr_offset that is non-triggering.
          tmpaddr0 = DMA_BASE + DMA_CH0_WRITE_ADDR_OFFSET +
                     (DMA_CH1_READ_ADDR_OFFSET * admachan0);
          tmpaddr1 = DMA_BASE + DMA_CH0_WRITE_ADDR_OFFSET +
                     (DMA_CH1_READ_ADDR_OFFSET * admachan1);
          // Dprintf("ADMA Maint %X %X %X %X %X
          // %X\n\r",amaddrs[0],amaddrs[1],tmpaddr0,tmpaddr1,&amaddrs[0],&amaddrs[1]);
          //                       channel, config, write_addr,
          //                       read_addr,transfer_count,trigger)
          dma_channel_configure(amaintchan0, &amcfg0, (uint32_t *)tmpaddr0,
                                &amaddrs[0], 1, false);
          dma_channel_configure(amaintchan1, &amcfg1, (uint32_t *)tmpaddr1,
                                &amaddrs[1], 1, false);
          adc_fifo_drain();
        } // adcdivint legal
      } // any analog enabled
      if (dev.d_mask) {
        ...
        // Due to how PIO shifts in bits, if any digital channel within a group
        // of 8 is set, then all groups below it must also be set. We further
        // restrict it in the tx_init function by saying digital channel usage
        // must be continous.
        /* pin count is restricted to 4,8,16 or 32, and pin count of 4 is only
       used Pin count is kept to a powers of 2 so that we always read a sample
       with a single byte/word/dword read for faster parsing. if analog is
       disabled and we are in D4 mode bits d_dma_bps   d_tx_bps 0-4    0 1 No
       analog channels 0-4    1          1        1 or more analog channels 5-7
       1          1 8      1          2 9-12   2          2 13-14  2          2
           15-16  2          3
           17-21  4          3
       */
        dev.pin_count = 0;
        ...
        if (dev.d_mask & 0x0FFF0000)
          dev.pin_count += 16;
        // If 4 or less channels are enabled but ADC is also enabled, set a
        // minimum size of 1B of PIO storage
        ...
        uint16_t div_int;
        uint8_t frac_int;
        div_int = frequency_count_khz(CLOCKS_FC0_SRC_VALUE_CLK_SYS) * 1000 /
                  dev.sample_rate;
        if (div_int < 1)
          div_int = 1;
        frac_int =
            (uint8_t)(((frequency_count_khz(CLOCKS_FC0_SRC_VALUE_CLK_SYS) *
                        1000 % dev.sample_rate) *
                       256ULL) /
                      dev.sample_rate);
        ...
        // Since we enable digital channels in groups of 4, we always get 32 bit
        // words
        ...
        // Since PIO transfers 32 bit values but DMA transfers 8, the d_size is
        // divided by 4. Dprintf("DMABufCfg d0_start %d d1_start %d d_size
        // %d\n\r",dev.dbuf0_start,dev.dbuf1_start,dev.d_size);
        //                     number    config   buffer target piosm xfer size
        //                     trigger
        dma_channel_configure(pdmachan0, &pcfg0,
                              &(capture_buf[dev.dbuf0_start]), &pio->rxf[piosm],
                              dev.d_size >> 2, true);
        dma_channel_configure(pdmachan1, &pcfg1,
                              &(capture_buf[dev.dbuf1_start]), &pio->rxf[piosm],
                              dev.d_size >> 2, false);
        // The maintenance DMA for PIO reads the capture_buff offset value and
        // updates the ADC DMAs with it
        pmaddrs[0] = (uint32_t *)&capture_buf[dev.dbuf0_start];
        pmaddrs[1] = (uint32_t *)&capture_buf[dev.dbuf1_start];
        // This is about as close to a common/portable address offset definition
        // between devices to find the offset of the write_addr_offset that is
        // non-triggering.
        tmpaddr0 = DMA_BASE + DMA_CH0_WRITE_ADDR_OFFSET +
                   (DMA_CH1_READ_ADDR_OFFSET * pdmachan0);
        tmpaddr1 = DMA_BASE + DMA_CH0_WRITE_ADDR_OFFSET +
                   (DMA_CH1_READ_ADDR_OFFSET * pdmachan1);
        // Dprintf("PDMA Maint %X %X %X %X %X
        // %X\n\r",pmaddrs[0],pmaddrs[1],tmpaddr0,tmpaddr1,&pmaddrs[0],&pmaddrs[1]);
        //                       channel, config, write_addr,
        //                       read_addr,transfer_count,trigger)
        dma_channel_configure(pmaintchan0, &pmcfg0, (uint32_t *)tmpaddr0,
                              &pmaddrs[0], 1, false);
        dma_channel_configure(pmaintchan1, &pmcfg1, (uint32_t *)tmpaddr1,
                              &pmaddrs[1], 1, false);
      ...
      // \n\r",&(capture_buf[dev.dbuf0_start]),&(capture_buf[dev.dbuf1_start]));
      // Dprintf("capture_buf analog %p
      // %p\n\r",&(capture_buf[dev.abuf0_start]),&(capture_buf[dev.abuf1_start]));
      // Dprintf("PIOSMCLKDIV 0x%X\n\r",*pio0sm0clkdiv);

      // Dprintf("PIO ctrl 0x%X fstts 0x%X dbg 0x%X lvl
      // 0x%X\n\r",*pioctrl,*piofstts,*piodbg,*pioflvl); Dprintf("DMA channel
      // assignments a %d %d d %d
      // %d\n\r",admachan0,admachan1,pdmachan0,pdmachan1);
      if (!adc_aborting) {
        // Clear any pending interrupts
        // All dma interrupts go through a common handler so that we can check
        // for overflows etc.
        ...
        // Dprintf("XY %X %X %X
        // %X\n",dev.d_mask,dev.a_chan_cnt,h0intmask,h1intmask); Enable logic
        // and analog close together for best possible alignment warning - do
        // not put printfs after this line as they will corrupt time measurement
        // and sample start
        tstart = time_us_32();
        dev.state = SENDING;

        // Pending Interrupts must be cleared immediately before enabling the
        // IRQ
        dma_hw->ints0 = dma_hw->ints0;
...
// it can cause code in the sdk to lock up serial CDC. These are rare
// noise/reset events and thus not checked when dev.started to ensure the
// maintenance loop runs as fast as possible.
#if (UART_EN == 1)
    if (dev.state == IDLE) {
    ...
    dcnt = time_us_32();
    // The '+' is the only character we track during normal sampling because it
    // can end a continuous trace or an aborted condition. A reset '*' should
    // only be seen after we have completed normally or hit an error condition.
    // The plus ends an aborted loop, is ignored by IDLE, and sends started to
    // IDLE. In all other cases the effect is not immediate and it's up to the
    // interrupt handler or send_half to make use of it.
    if (usbintin == '+') {
      // Dprintf("USB plus\n\r");
    ...
    // send_resp is set to true but not processed immediately so we can get back
    // to send_half ASAP
    else if (usbintin >= 0) {
      bcnt++;
    ...
    // The libsigrok processing of aborts is not clean, and may try to report
    // the "!" as a bad rle value.
    // Emperically it seems that waiting for a long delay to ensure the host has
    // drained it's Rx buffere and then sending 3 "!" is the most reliable way
    // to get it to exit. As an additional failsafe, a final bytecnt with a
    // count of 0 is sent. In forced test mode we also don't get the usb plus,
    // so the forced exit on abort covers that as well
    if (dev.state == ABORTED) {
      Dprintf("sending abort! ftm %d num_halves %d dma_halves %d sho cnt %d "
              "tx_cnt %d\n\r",
    ...
    // Once we reach SAMPLES_SENT, send the final byte count to ensure no bytes
    // were lost
    if (dev.state == SAMPLES_SENT) {
      ...
      puts_raw(brsp);
      // Print out debug information after completing, rather than before so
      // that it doesn't delay the start of a capture
      Dprintf("Complete: SRate %d NSmp %d NHalves %d\n\r", dev.sample_rate,
              dev.num_samples, num_halves);
      ...
      Dprintf("Half buffers exp %d DMA %d Sent %d sampperhalf %d\n\r",
              exp_halves, dma_halves, num_halves, dev.samples_per_half);
      dev.state = IDLE;
#ifdef PIN_TEST_MODE
      for (int y = 0; y < SYSTICK_PRINT; y++) {
        ...
        if (delta > 120) {
          Dprintf("*****Systick Anomoly ST%3d %d %d\n\r***", y,
                  systick_array[y], delta);
        }
      }
#endif // PIN_TEST_MODE
    }
    // Since there are so many FSM arcs, it's safest to just continually clear
    // state if we aren't sending
    if (dev.state == IDLE) {
      // forced_test_mode is really a one shot deal as there is no way to
      // restart it. Exit the mode so that host accesses will work normally
      forced_test_mode_run = false;
#ifdef BASE_MODE
      adc_run(false);
      adc_fifo_drain();
...
// This HW based trigger which should be part of send slices was tested enough
// to confirm the trigger value worked, however it was not fully implemented
// because the RP2040 wasn't able to perform the trigger detection and memory
// buffer management to support sample rates that were substantially higher than
// the stream rates across USB.  Thus there wasn't a compelling reason to have
// it. It's left as an example as to how the masks could be used.
// To fully support a HW based triggering, a precapture ring buffer of both
// digital and analog samples
// would need to be created an managed to store and send pretrigger values.
// The ring buffer would need to support RLEs and would need to ensure it was
// sent before sending other samples capture by the DMA after the trigger event.
void depracated_trigger_logic(void) {
  uint32_t tlval; // Trigger last val
  tlval = d->tlval;
  uint32_t all_mask =
      d->lvl0mask | d->lvl1mask | d->risemask | d->fallmask | d->chgmask;
  ...
  if (d->triggered == false) {
    ...
    if (matches == all_mask) {
      ...
      d->triggered = true;
      // This sends the last val on a trigger because SW based trigger on
      // the host needs to see its
