...
int main() {
  ...
  gpio_set_dir_masked(
      GPIO_D_MASK, GPIO_D_MASK); // TODO - I think an out can still be an ind?
  ...
  while (true) {
    ...
    // cval=(ctime-tstart)<<4; //not a true  62.5ns/8Mhz wave, but will
    // increment as such cval=(ctime-tstart)<<2; //not a true 250ns/2Mhz wave,
    // but will increment as such cval=(ctime-tstart); //1us, 500kz
    ...
    // cval=0x123456789;
    // Note: both the mask and the value must be shifted up by the starting
    // number of bits
